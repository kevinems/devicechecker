...
public class SleepWakeActivity extends Activity implements OnClickListener{
    ...
    public void onCreate(Bundle savedInstanceState) {
			...
			public void onClick(View v) {
				...
				finish();
				 ...
				 if (FileOperate.getCurMode()==FileOperate.TEST_MODE_ALL){
					 Intent mIntent = FileOperate.getCurIntent(SleepWakeActivity.this,"Sleep-Awake");
					 if (mIntent!=null) {
		...
		if (FileOperate.getCurMode()==FileOperate.TEST_MODE_ALL){
		FileOperate.setIndexValue(FileOperate.TestItemClose, FileOperate.CHECK_FAILURE);  
		FileOperate.writeToFile(this);
		}
    }
	 ...
	 Runnable mRunnable = new Runnable() {	
		...
		public void run() {
			if (checkWifiIscon()) {
				...
				FileOperate.MyToast(getApplicationContext(), null, R.string.browserconnect);
			}else {
				mHandler.postDelayed(mRunnable, Check_Time);
			}
		}
